        ...
        <div style={{ padding: 28, display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 760 }}>
          ...
          {result && !analyzing && (
            <div style={{ background: PANEL_AI, border: `1px solid ${BORDER_AI}`, borderRadius: 16, padding: '22px 24px', backdropFilter: 'blur(8px)' }}>
                ...
                <div style={{ marginBottom: 18 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {result.line_items.map((it, i) => (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 12px', background: '#141417', borderRadius: 8, border: `1px solid ${BORDER}`, fontSize: 13 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={() => { setResult(null); setFileName('') }} style={{ background: 'transparent', color: SUB, border: `1px solid ${BORDER}`, padding: '11px 20px', borderRadius: 9, fontSize: 14, cursor: 'pointer' }}>
                  Discard
                </button>
              </div>
            </div>
          ...
          {saved.length > 0 && (
            <>
              ...
              {saved.map((po) => (
                <div key={po.id} style={{ background: PANEL, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '16px 18px', backdropFilter: 'blur(8px)' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                        <span style={{ color: INK, fontSize: 15, fontWeight: 600 }}>{po.vendor ?? 'Unknown vendor'}</span>
                        {po.po_number && <span style={{ fontSize: 11, color: BLUE_TEXT, background: '#0F1E33', border: '1px solid #234A7A', padding: '2px 8px', borderRadius: 20 }}>PO #{po.po_number}</span>}
                      </div>
